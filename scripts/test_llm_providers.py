"""
[SCRIPT] Zero-Cost LLM Provider Verification Script
Tests each provider individually with strict rate limiting.

NOTA: Este archivo es un SCRIPT de verificación manual, NO un módulo de tests
de pytest. Para que pytest no lo recolecte (y no rompa el discovery importando
litellm/dotenv a nivel módulo), los imports pesados son perezosos (dentro de
las funciones) y las funciones NO siguen el patrón `test_*`.
...
# ============================================================
# LAZY SETUP (only when run as a script, never during pytest collection)
# ============================================================

def _load_env():
    """Carga .env perezosamente (solo cuando se ejecuta como script)."""
    try:
        from dotenv import load_dotenv
    except Exception:
        print("[WARNING] python-dotenv no instalado; las API keys deben estar en el entorno.")
        return
    root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    for name in ("VeriAgent.env", ".env"):
        env_file = os.path.join(root, name)
...
# ============================================================
# CHECK FUNCTIONS (prefixed `check_` to avoid pytest collection)
# ============================================================

def check_provider_litellm(name: str, model: str, api_key_env: str) -> dict:
    """
    Tests a provider via LiteLLM.
    """
    import litellm  # lazy import — no rompe la colección de pytest
...
def run_all_checks():
    ...
    for name, model, env_key in litellm_providers:
        result = check_provider_litellm(name, model, env_key)
        results[name] = result
    ...
    gemini_result = check_gemini_native()
